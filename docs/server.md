...
- Each photo gets three aggregate attempts: Flickr original `_o`, `Large 2048` `_k`, then `Large 1024` `_b`. Every CDN request uses the tested browser User-Agent, image `Accept`, cache-busting, and a 60-second request timeout.
