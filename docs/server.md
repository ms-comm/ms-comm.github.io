...
- Each photo gets three aggregate attempts: Flickr original `_o` from owner metadata, exact `Large 2048` `_k`, then exact `Large 1024` `_b` URLs returned by Flickr. Every CDN request uses the tested browser User-Agent, image `Accept`, cache-busting, and a 60-second request timeout.
