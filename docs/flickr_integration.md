...
Asynchronous album ZIP jobs therefore use that browser User-Agent and exactly three distinct CDN sources per photo: original `_o` built from owner-authenticated `getInfo`, exact `Large 2048` `_k` returned by `getSizes`, then the validated `_b` URL returned by `getInfo`. Exact-label and suffix validation prevents `getSizes` from silently returning its widest fallback or repeating a URL; manually constructed `_k` URLs are forbidden because real production probes showed Flickr can return `410` for them. The `_b` source does not depend on Flickr's variable 1024-size label. Requests include image `Accept`, cache-busting, and a 60-second timeout. Failures wait 5 then 10 seconds; the third failure marks the job `failed` and stops it permanently. Polling never resets attempts or restarts a failed job.
