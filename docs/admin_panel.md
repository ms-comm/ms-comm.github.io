...
- **Named access (`#album-access-fields`)**: for a **saved** album of a private type, the album modal lists the people who may open it without the code. Add by e-mail (`#album-access-email` + `#album-access-add`), revoke from `#album-access-list`. `toggleAccessFields()` hides the block for a new or non-private album; `openEditAlbum` clears then reloads through `loadAlbumAccess()`, and `togglePrivateFields` re-evaluates it on every type change. The grant is keyed by normalised e-mail, so granting to someone who has no account yet is valid — the album appears in their « Mon espace → Mes albums partagés » as soon as they sign up with that address. Backing store: `db/album-grants.json` (see [server.md](server.md)).
