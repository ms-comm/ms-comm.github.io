...
- A dedicated album progress panel reports local photo count, local ZIP creation, server fallback, and server progress. A terminal server failure immediately shows the backend reason, clears saved tracking, hides progress, and re-enables the button for an explicit manual retry.
