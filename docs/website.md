...
- **Préparation** has no countable photo unit (it is one manifest request that resolves every Flickr URL server-side and can be slow on a big album), so it is reported as 3 explicit sub-steps — access check, server building the list, list ready — and never sits on an empty frozen bar. If a step ever runs with `total = 0`, the controller drops the `value` attribute to render the native indeterminate bar and pulses the label so the user can see it is alive.
