...
- Mobile downloads: the lightbox `Télécharger` action becomes an icon-only circular button below 600px, while album download controls keep icon + text. Album choice menus are positioned from the tapped button with a viewport clamp so they never open partly outside the phone. The single-photo modal uses a large contain-fit watermarked preview on the left and stacked format radio cards plus one contextual action on desktop; mobile keeps the thumbnail/title header, stacked radio choices and shared action, with the paid original selected by default. The preview always stays watermarked, and the original price comes from the photo record.
