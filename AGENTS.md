...
- **Lossless re-migration caveat (wmOnly rows)**: the export equals CDN `_o` byte for byte (sha checked). For a watermark-only photo whose export is already stamped, `wm`/`display` carry a second logo (master stays as published). No reliable pixel detector exists (23 % false positives), so control a visual sample of the 2026-04 albums after migration. 124 wmOnly exports carry EXIF orientation 6/8: `.rotate()` stays mandatory on the `wm`/`display` branches; the raw master keeps its EXIF and browsers straighten it.
