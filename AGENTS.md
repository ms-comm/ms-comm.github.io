...
- **ZIP Flickr fallback**: Use the tested browser User-Agent and cache-busting for three distinct sources: original `_o` from owner `getInfo`, then exact `Large 2048` `_k` and exact `Large 1024` `_b` URLs returned by `getSizes`. Never manually invent intermediate-size URLs or accept `getSizes` widest fallback. Strict watermark albums resolve all three from the watermark Flickr photo ID only.
