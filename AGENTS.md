...
### 2026-09-17 account deep-link refresh fix

- `photos.html` binds its `MSAccount.onChange` listeners after deferred account scripts have initialized. The current « Mon espace » view is repainted immediately and again on session/favorites changes, so `?view=favorites`, `?view=albums_partages` and `?view=purchased` no longer stay on the signed-out first-connection state.
...
Cross-site account persistence: GitHub Pages → Fly can lose third-party cookies on mobile. Account login/session payloads therefore also issue a signed 30-day `accountToken`; `assets/js/account.js` stores it locally and sends it only as a Bearer token to `/api/account/*`. Download authorization remains the separate 10-minute `dlTicket`.
...
Responsive gallery/checkout: below 600px the single-photo lightbox download control is icon-only, but album download controls remain icon + text. Album download menus are viewport-clamped from the tapped button and the lightbox download modal has a viewport-safe width. `checkout.html` must keep grid children and the Stripe payment iframe shrinkable (`min-width:0`, `max-width:100%`) so the payment card never clips horizontally; name fields stack below 380px. Details: [docs/website.md](docs/website.md).
...
- **Persistent login**: visitor and admin sessions default to 30 days; login handlers flush the regenerated session before returning the cookie. `remember:false` keeps the 8-hour short session.
...
- **Purchased ZIP source**: `/api/orders/:id/download-all` is server-generated only from local originals or R2 master, never Flickr. A legacy row without a server source returns `409 ZIP_SOURCE_NOT_READY` until migrated; unique stored extensions are preserved and incomplete archives are never cached. Admin payment audit is read-only at `/api/admin/orders/payment-audit` and is exposed by “Vérifier Stripe”.
...
- **Checkout payment intent**: Never create a Stripe PaymentIntent with a placeholder email. The frontend creates it only after a valid customer email is entered, then confirms it after consent to the terms/privacy links.
- **Mobile download controls**: the single-photo lightbox download button is icon-only under 600px, while album/order download buttons keep icon + text at every width. Album menus are viewport-clamped and checkout payment content must shrink to the available mobile card width. Accessible `aria-label`/`title` values remain. Cart icon uses flex centering with no baseline offset.
- **Single-photo download modal**: desktop uses a large contain-fit watermarked preview on the left and vertically stacked format radio cards plus one contextual action on the right. Under 700px keep the compact thumbnail/title header and stacked radio choices, with the paid original selected by default. The preview stays watermarked for either choice; purchase price comes from the photo record.
- **Compliance pages**: `cgv.html`, `confidentialite.html` and `mentions-legales.html` are linked from checkout. They contain the technical/legal structure but the operator must fill legal identity, SIRET/VAT, address, mediator and retention details before commercial activation.
...
- **Contact service selection**: `contact.html` renders removable service chips from `?pack=` links, supports multiple packs/services and `Projet personnalisé`, and includes the selection in the generated mailto subject/body. Event photography is listed in the Photographies card as `Photo d'événement — Prix à discuter`.
