...
Cloudflare R2 is the new photo storage. Three levels per photo: `master/` (4096 px q90, private bucket `ms-comm-master`), `wm/` (full-res watermarked, private, built lazily on first download, never pre-rendered by the migration) and `display/` (1600 px q82, public bucket `ms-comm-display`). A photo is served from R2 as soon as it carries `r2Key`; Flickr stays a read fallback until the migration completes. Clearing `r2Key` reverts a photo to Flickr instantly — nothing is deleted on Flickr.
...
- **R2 storage levels**: `master` 4096 px q90 (~1,2 Mo/photo, ~4,8 Go for 3887 — inside the 10 Go free tier). Storing raw Flickr masters was measured at 9,43 Mo/photo → 46 Go, which is why the master is re-encoded. `wm` is built on first watermarked download and kept; pre-rendering it for every photo would roughly double the stored volume for files most of which are never downloaded.
...
- **The `wm` level must be pre-built, not left lazy**: a photo with a clean original has no `wm` object until someone downloads it watermarked, and until then `download-urls` cannot presign and silently falls back to the Flickr CDN — 120 of the 121 photos of the paid album still went to Flickr after the migration, keeping exactly the slowness and 429 risk R2 was meant to remove. `tools/prebuild-wm.js` builds them from the R2 master (no Flickr call, 564 photos, +0,71 Go, ~3 min) and must be re-run after any upload of photos carrying `flickrWatermarkId`. It refuses to start without the watermark assets, like the migration.
