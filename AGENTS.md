...
- **Purchased ZIP fallback**: `/api/orders/:id/download-all` keeps local → R2 master → Flickr-original fallback for legacy paid rows without `r2Key`, uses unique stored extensions, and never caches an incomplete archive. Admin payment audit is read-only at `/api/admin/orders/payment-audit` and is exposed by “Vérifier Stripe”.
